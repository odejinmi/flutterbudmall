import '../common/my_strings.dart';
import 'bank.dart';

class CheckoutResponse {
  /// A user readable message. If the transaction was not successful, this returns the
  /// cause of the error.
  String message;

  /// The card used for the payment. Will be null if the customer didn't use card
  /// payment
  // PaymentCard? card;

  /// The bank account used for the payment. Will be null if the customer didn't use
  /// bank account as a means of  payment
  BankAccount? account;

  /// Transaction reference. Might be null for failed transaction transactions
  String? reference;

  /// The status of the transaction. A successful response returns true and false
  /// otherwise
  bool status;
  bool loading;

  /// The means of payment. It may return [CheckoutMethod.bank] or [CheckoutMethod.card]
  // CheckoutMethod method;
  // CheckoutPage page;

  /// If the transaction should be verified. See https://developers.budmall.co/v2.0/reference#verify-transaction.
  /// This is usually false for transactions that didn't reach Budmall before terminating
  ///
  /// It might return true regardless whether a transaction fails or not.
  bool verify;

  CheckoutResponse.defaults()
      : message = Strings.userTerminated,
        status = false,
        verify = false,
        loading = true;
  // method = CheckoutMethod.selectable,
  // page = CheckoutPage.selectable;

  CheckoutResponse(
      {required this.message,
      required this.reference,
      required this.status,
      // required this.method,
      // required this.page,
      required this.verify,
      required this.loading,
      // this.card,
      this.account})
      : assert(account != null);

  @override
  String toString() {
    // return 'CheckoutResponse{message: $message, card: $card, account: $account, reference: $reference, status: $status, method: $method, verify: $verify, page: $page, loading: $loading}';
    return 'CheckoutResponse{message: $message, account: $account, reference: $reference, status: $status, verify: $verify, loading: $loading}';
  }
}
